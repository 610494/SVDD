...
from .lang_emb_LeNet_len_free_nisqa import LangEmbLeNetLenFreeNisqa, LangEmbLeNetLenFreeNisqaAutoencoder
from .lang_emb_LeNet_vq import LangEmbLeNetVq, LangEmbLeNetAutoencoderVq
from .lang_emb_LeNet_len_free_vq import LangEmbLeNetLenFreeVq, LangEmbLeNetLenFreeVqAutoencoder

def build_network(net_name):
    """Builds the neural network."""

    implemented_networks = ('mnist_LeNet', 'cifar10_LeNet', 'cifar10_LeNet_ELU', 'lang_emb_LeNet', 'lang_emb_LeNet_no_SIG', 'lang_emb_LeNet_ssl', 'lang_emb_LeNet_dns_ssl', 'lang_emb_LeNet_nisqa_ssl', 'lang_emb_LeNet_len_free', 'lang_emb_LeNet_len_free_nisqa', 'lang_emb_LeNet_vq', 'lang_emb_LeNet_len_free_vq')
    ...
    if net_name == 'lang_emb_LeNet_len_free_nisqa':
        net = LangEmbLeNetLenFreeNisqa()
    
    if net_name == 'lang_emb_LeNet_vq':
        net = LangEmbLeNetVq()
    
    if net_name == 'lang_emb_LeNet_len_free_vq':
        net = LangEmbLeNetLenFreeVq()
    return net
...
def build_autoencoder(net_name):
    ...
    if net_name == 'lang_emb_LeNet_len_free_nisqa':
        ae_net = LangEmbLeNetLenFreeNisqaAutoencoder()
    
    if net_name == 'lang_emb_LeNet_vq':
        ae_net = LangEmbLeNetAutoencoderVq()
    
    if net_name == 'lang_emb_LeNet_len_free_vq':
        ae_net = LangEmbLeNetLenFreeVqAutoencoder()
    return ae_net