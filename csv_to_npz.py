...
# '/mnt/md1/user_wago/MOS/csv/matbn_enhanced_split_3s.csv',
# '/mnt/md1/user_wago/MOS/csv/matbn_enhanced_split_5s.csv'

# '/mnt/md1/user_wago/MOS/csv/distortion_SNR_10dB.csv'
# '/mnt/md1/user_wago/MOS/csv/LJSpeech_SNR_10dB_split_5s.csv'
# '/mnt/md1/user_wago/MOS/csv/LJ_random.csv'
file_paths = [
    # '/mnt/md1/user_wago/MOS/csv/distortion_SNR_10dB.csv',
    # '/mnt/md1/user_wago/MOS/csv/LJSpeech_SNR_10dB_split_5s.csv',
    # '/mnt/md1/user_wago/MOS/csv/LJ_random.csv'
...
for file_path in file_paths:
    ...
    features_len_free = df.drop(['filename', 'len_in_sec', 'OVRL_raw', 'SIG_raw', 'BAK_raw', 'OVRL', 'SIG', 'BAK', 'P808_MOS', 'mos_pred', 'col_pred','loud_pred', 'dis_pred', 'MOS_COL', 'MOS_LOUD','MOS_REVERB','MOS_SIG', 'MOS_OVRL'], axis=1).values
    ...
    # features_len_free_wt_vq = df.drop(['filename', 'len_in_sec', 'OVRL_raw', 'SIG_raw', 'BAK_raw', 'OVRL', 'SIG', 'BAK', 'P808_MOS', 'mos_pred', 'col_pred','loud_pred', 'dis_pred', 'MOS_COL','MOS_DISC','MOS_LOUD','MOS_REVERB','MOS_SIG'], axis=1).values
    # features_ssl_only = df.drop(['filename', 'len_in_sec', 'OVRL_raw','SIG_raw','BAK_raw','OVRL','SIG','BAK','P808_MOS','mos_pred','noi_pred','dis_pred','col_pred','loud_pred','MOS_COL','MOS_DISC','MOS_LOUD','MOS_NOISE','MOS_REVERB','MOS_SIG','MOS_OVRL'], axis=1).values
    
    # features_len_free_nisqa = df.drop(['filename', 'len_in_sec', 'MOS_COL','MOS_LOUD','MOS_REVERB','MOS_SIG','MOS_OVRL', 'OVRL_raw', 'SIG_raw', 'BAK_raw', 'OVRL', 'SIG', 'BAK', 'P808_MOS'], axis=1).values
    ...
    # npz_file_path_features_ssl_only = file_path.replace('.csv', '_ssl_only.npz') 
    
    # npz_file_path_len_free_wt_vq = file_path.replace('.csv', '_len_free_wt_vq.npz') 
    
    # npz_file_path_len_free_nisqa = file_path.replace('.csv', '_len_free_nisqa.npz') 
    ...
    # np.savez(npz_file_path_len_free_wt_vq, ids=ids, features=features_len_free_wt_vq)
    np.savez(npz_file_path_len_free, ids=ids, features=features_len_free)
    # np.savez(npz_file_path_features_ssl_only, ids=ids, features=features_ssl_only)
    # np.savez(npz_file_path_len_free_nisqa, ids=ids, features=features_len_free_nisqa)
