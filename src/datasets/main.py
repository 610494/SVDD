...
from .lang_emb_len_free_nisqa import LangEmbLenFreeNisqaDataset
from .lang_emb_vq import LangEmbDatasetVq
from .lang_emb_len_free_nisqa_vq import LangEmbLenFreeNisqaVqDataset

def load_dataset(dataset_name, data_path, normal_class):
    """Loads the dataset."""

    implemented_datasets = ('mnist', 'cifar10', 'lang_emb', 'lang_emb_no_sig', 'lang_emb_ssl', 'lang_emb_dns_ssl', 'lang_emb_nisqa_ssl', 'lang_emb_len_free', 'lang_emb_len_free_nisqa', 'lang_emb_vq', 'lang_emb_len_free_nisqa_vq')
    ...
    if dataset_name == 'lang_emb_len_free_nisqa':
        dataset = LangEmbLenFreeNisqaDataset(root=data_path, normal_class=normal_class)
    
    if dataset_name == 'lang_emb_vq':
        dataset = LangEmbDatasetVq(root=data_path, normal_class=normal_class)
    
    if dataset_name == 'lang_emb_len_free_nisqa_vq':
        dataset = LangEmbLenFreeNisqaVqDataset(root=data_path, normal_class=normal_class)
    return dataset