...
class LangEmbLenFreeDataset(TorchvisionDataset):
    def __init__(self, root: str, normal_class: str):  # Original: __init__(self, normal_class: str)
        ...
        train_set = MyLangEmb(npz_file_path=os.path.join( # to change for train!
            '../aishell3_train_len_free.npz'), normal_class=normal_class)
        self.train_set = train_set

        test_set = MyLangEmb(npz_file_path=os.path.join( # to change for test and inference!
            '../matbn_enhanced_len_free.npz'), normal_class=normal_class)
        self.test_set = test_set
        # LJSpeech_len_free.npz
        # LJSpeech_split_5s_len_free.npz
        # LJ_random_split_1s_len_free.npz_len_free.npz
...
class MyLangEmb(Dataset):
    ...
    def __len__(self):
        ...
        # print(f'type(self.npz_file["features"]): {type(self.npz_file["features"])}')
        # input("")
